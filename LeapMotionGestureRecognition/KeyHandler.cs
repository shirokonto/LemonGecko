...
namespace LeapMotionGestureMapper
{
    public class KeyHandler
    {
        ...
        private List<Process> _nvdaProcesses;
        ...
        {
            _nvdaProcesses = new List<Process>();
            _nvdaProcesses.AddRange(Process.GetProcessesByName("Chrome"));
        ...
        {
            if (isCurrentWindowNvda())
            {
                foreach (Process proc in _nvdaProcesses)
                {
        ...
        private bool isCurrentWindowNvda()
        {
            GetWindowThreadProcessId(GetForegroundWindow(), out _foregroundWindowProcessId);
